...
//! the control rather than the items because the items exist only while the
...
//!
//! Both selectors are `controls/drop.rs`'s drop-down rather than egui's
//! `ComboBox` (bl-78c2), for one reason and only one: a combo's list is
//! placed against the display, and the display includes the gesture-nav zone.
...
impl Shell {
    ...
    pub(super) fn models(
        ...
        act::acts(ui, &opened, &["models", "model"]);
        if let Some(name) = picked {
